...
def fn(x):
    return (torch.sin(2 * torch.pi * x) + 1) / 2
...
def compute_curve(ctrl_x: np.ndarray, k: int, num_samples: int = 600):
    ...
    x = torch.linspace(0.0, 1.0, steps=num_samples)  # [N]
    x_ = x.unsqueeze(1)  # [N, 1]
    value = eval_basis_functions(x_, grid, int(k), extend=True).squeeze(dim=1)  # [N, H]
    # find the coefficients
    coeff = torch.linalg.lstsq(value, fn(x)).solution  # [H]
    y_pred = torch.einsum("i, ji -> j", coeff, value)
    return x.numpy(), y_pred.numpy()
